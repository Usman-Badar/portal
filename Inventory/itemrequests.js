...
router.post('/inventory/get_item_request/get_sub_category_items', ( req, res ) => {
    ...
    db.query(
        ...
        LEFT OUTER JOIN companies ON tbl_inventory_products.company_code = companies.company_code \
        WHERE tbl_inventory_products.sub_category_id = ? AND tbl_inventory_products.employee IS NULL;",
        [ sub_category_id ],
        ( err, rslt ) => {
...
router.post('/inventory/item_request/deliver_to_employee', ( req, res ) => {
    ...
    const { assigned_products, request_id, requested_by } = req.body;
    ...
    db.query(
        ...
        ( err ) => {
            ...
            }else 
            {
                ...
                {
                    itemRequestDescriptionQuery = itemRequestDescriptionQuery.concat("UPDATE `tbl_inventory_products` SET tbl_inventory_products.quantity = tbl_inventory_products.quantity - ? WHERE tbl_inventory_products.product_id = ?;");
