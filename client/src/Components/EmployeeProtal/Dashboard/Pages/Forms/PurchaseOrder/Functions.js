...
const additionalCost = ( e, value_in_amount ) => {
    ...
    )
    {
        ...
        let value_in_amount_int = parseInt(value_in_amount.textContent == '' ? 0 : value_in_amount.textContent);
...
export const SubTotalCostCalculation = () => {
    ...
    document.getElementById('sub_total_calculated_amount').textContent = total_calculated_amount;
...
export const TotalCostCalculation = () => {
    ...
    for ( let x = 0; x < rows_array.length; x++ )
    {
        ...
        {
            total_calculated_amount = total_calculated_amount + parseInt(label_value_column.textContent);
        }
    }

    document.getElementById('total_calculated_amount').textContent = parseFloat(sub_total) + parseFloat(total_calculated_amount);
    return parseFloat(sub_total) + parseFloat(total_calculated_amount);

}

export const loadPRList = ( setPRList, setPRSPec ) => {

    axios.post('/purchase/requisition/load/requests_with_specifications', { emp_id: sessionStorage.getItem('EmpID') }).then(
        res => {

            setPRList( res.data[0] );
            setPRSPec( res.data[1] )
...
export const onAttachBills = ( event, setBills ) => {
...
export const openRequestDetails = ( po_id, setRequestDetails, setSpecifications, setBills, setAdditionalRows, setPRequestDetails, setPRSpecifications ) => {
    ...
    .then(
        res => 
        {
            ...
            setSpecifications(res.data[0][2]);
